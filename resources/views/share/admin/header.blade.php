    ...
    <nav class="header-navbar navbar-expand-lg navbar navbar-fixed align-items-center navbar-shadow navbar-brand-center"
        ...
        <div class="navbar-header d-xl-block d-none">
            <ul class="nav navbar-nav">
                <li class="nav-item"><a class="navbar-brand" href="{{ Route('indexDashboard') }}"><span class="brand-logo">
        ...
        <div class="navbar-container d-flex content">
            <ul class="nav navbar-nav align-items-center ms-auto">
                <li class="nav-item dropdown dropdown-notification me-25"><a class="nav-link" href="#"
                        ...
                        </svg><span class="badge rounded-pill bg-danger badge-up">@{{ list.length }}</span></a>
                    <ul class="dropdown-menu dropdown-menu-media dropdown-menu-end">
                        <li class="dropdown-menu-header">
                            <div class="dropdown-header d-flex">
                                <h4 class="notification-title mb-0 me-auto">Thông Báo</h4>
                                <div class="badge rounded-pill badge-light-primary">@{{ list.length }} New</div>
                            </div>
                        </li>
                        <template v-for='(v,k) in list'>
                            <li class="scrollable-container media-list ps">
                                <a class="d-flex" :href="'/admin/notification/abc/' + v.id">
                                    <div class="list-item d-flex align-items-start">
                                        <div class="list-item-body flex-grow-1">
                                            <p class="media-heading">
                                                <span class="fw-bolder">
                                                    @{{ v.ho_va_ten }}
                                                </span>đã gửi liên hệ cho bạn!
                                            </p>
                                            <small class="notification-text">
                                                @{{ v.loi_nhan.length > 40 ? forMat(v.loi_nhan, 40) + '...' : v.loi_nhan }}
                                            </small>
                                        </div>
                                    </div>
                                </a>
        ...
        new Vue({
            ...
            methods: {
                ...
                forMat(text, maxLength) {
                    ...
                    // Tìm vị trí cuối cùng của khoảng trắng trước maxLength
                    const lastSpace = text.lastIndexOf(' ', maxLength);
                    // Nếu không tìm thấy khoảng trắng, cắt tại maxLength
                    const truncated = lastSpace !== -1 ? text.substring(0, lastSpace) : text.substring(0,
                        maxLength);
                    return truncated + '...';
                },
            },
